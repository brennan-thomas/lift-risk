...
def heatmap(mat, title, file, size=4):
    dir = os.path.dirname(file)
    if len(dir) > 0:
        os.makedirs(dir, exist_ok=True)
    ...
    def get_acc(i):
        return str(round(mat[i,i] / mat_sum[i], 2))
    annot = np.diag([get_acc(i) for i in range(size)])

    total_accuracy = sum([mat[i, i] for i in range(size)]) / mat_sum.sum()
    ...
    if size == 4:
        ...
        ax.set_xticklabels(list(map(str, range(1, size+1))))
        ax.set_yticklabels(list(map(str, range(1, size+1))))
    f.tight_layout()
...
def binary(mat, title, file):
    ...
    def get_acc(i):
        return str(round(mat[i,i] / mat_sum[i], 2))
    annot = np.diag([get_acc(i) for i in range(2)])

    total_accuracy = sum([mat[i, i] for i in range(2)]) / mat_sum.sum()
